...
const MembersList = ({ newMuteList }) => {
    ...
    const isAdmins =
        groupAdmins.includes(loginUser) || loginUser === groupOwner;
        ...
        <Box>
            {newMembers.length > 0 &&
                newMembers.map((item, key) => {
                    let owner =
                        groupOwner === item ? `${i18next.t("owner")}` : "";
